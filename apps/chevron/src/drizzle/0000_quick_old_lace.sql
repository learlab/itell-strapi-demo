...
CREATE TABLE IF NOT EXISTS "user_model" (
	"id" serial PRIMARY KEY NOT NULL,
	"user_id" text NOT NULL,
	"summary_streak" integer NOT NULL,
	"CRI_streak" integer NOT NULL,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL
);
--> statement-breakpoint
...
--> statement-breakpoint
DO $$ BEGIN
 ALTER TABLE "user_model" ADD CONSTRAINT "user_model_user_id_users_id_fk" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE cascade ON UPDATE cascade;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
--> statement-breakpoint
