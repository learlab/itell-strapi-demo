...
<Notebook code = {`let ten = 10;
console.log(ten * ten);
// → 100`}/>

When a binding points at a value, that does not mean it is tied to that value forever. The = operator can be used at any time on existing bindings to disconnect them from their current value and have them point to a new one:

<Notebook code = {`let mood = "light";
console.log(mood);
// → light
mood = "dark";
console.log(mood);
...
<Notebook code = {`let luigisDebt = 140;
...
<Notebook code = {`break case catch class const continue debugger default
...
<Notebook code = {`prompt("Enter passcode");`}/>
...
<Notebook code = {`console.log(Math.max(2, 4));
// → 4`}/>
...
<Notebook code = {`console.log(Math.min(2, 4) + 100);
// → 102`}/>
...
<Notebook code = {`let theNumber = Number(prompt("Pick a number"));
...
![2-2-controlflow-straight.svg](https://nbjrajrmujlgxmcvqsge.supabase.co/storage/v1/object/public/strapi/files/2-2-controlflow-straight.svg-a3e3034abbe58c2c59ada45b67e9964e.svg)
</section>
...
![2-3-controlflow-if.svg](https://nbjrajrmujlgxmcvqsge.supabase.co/storage/v1/object/public/strapi/files/2-3-controlflow-if.svg-ee294c8069b422f4f4f349d52cc2f89f.svg)
...
<Notebook code = {`if (1 + 1 == 2) console.log("It's true");
// → It's true`}/>
...
<Notebook code = {`let num = Number(prompt("Pick a number"));
if (num < 10) {
 console.log("Small");
} else if (num < 100) {
 console.log("Medium");
...
![2-4-controlflow-nested-if.svg](https://nbjrajrmujlgxmcvqsge.supabase.co/storage/v1/object/public/strapi/files/2-4-controlflow-nested-if.svg-9a871b16fe2e8b548cadd147eb11bd2a.svg)
</section>
...
![2-5-controlflow-loop.svg](https://nbjrajrmujlgxmcvqsge.supabase.co/storage/v1/object/public/strapi/files/2-5-controlflow-loop.svg-7d489d1b98224d28fec7d0cdad6d7997.svg)
...
<Notebook code = {`for (let number = 0; number <= 12; number = number + 2) {
 console.log(number);
}
// → 0
// → 2
...
<Notebook code = {`for (let current = 20; ; current = current + 1) {
 if (current % 7 == 0) {
   console.log(current);
   break;
 }
...
<Notebook code = {`counter = counter + 1;`}/>



JavaScript provides a shortcut for this:

<Notebook code = {`counter += 1;`}/>
...
<Notebook code = {`for (let number = 0; number <= 12; number += 2) {
 console.log(number);
}`}/>
...
<Notebook code = {`if (x == "value1") action1();
...
<Notebook code = {`switch (prompt("What is the weather like?")) {
 case "rainy":
...
<Notebook code = {`fuzzylittleturtle
fuzzy_little_turtle
FuzzyLittleTurtle
fuzzyLittleTurtle`}/>
...
<Notebook code = {`let accountBalance = calculateBalance(account);
...
Write a loop that makes seven calls to <Notebook code = {`console.log`}/> to output the following triangle:
...
It may be useful to know that you can find the length of a string by writing <Notebook code = {`.length`}/> after it.
...
You can start with a program that prints out the numbers 1 to 7, which you can derive by making a few modifications to the [even number printing example](https://eloquentjavascript.net/02_program_structure.html#loops)given earlier in the chapter, where the <Notebook code = {`for`}/> loop was introduced.

Now consider the equivalence between numbers and strings of hash characters. You can go from 1 to 2 by adding 1 (<Notebook code = {`+= 1`}/>). You can go from <Notebook code = {`"#"`}/> to <Notebook code = {`"##"`}/> by adding a character (<Notebook code = {`+= "#"`}/>). Thus, your solution can closely follow the number-printing program.

### **FizzBuzz**

Write a program that uses <Notebook code = {`console.log`}/> to print all the numbers from 1 to 100, with two exceptions. For numbers divisible by 3, print <Notebook code = {`"Fizz"`}/> instead of the number, and for numbers divisible by 5 (and not 3), print <Notebook code = {`"Buzz"`}/> instead.

When you have that working, modify your program to print <Notebook code = {`"FizzBuzz"`}/> for numbers that are divisible by both 3 and 5 (and still print <Notebook code = {`"Fizz"`}/> or <Notebook code = {`"Buzz"`}/>for numbers divisible by only one of those).
...
Going over the numbers is clearly a looping job, and selecting what to print is a matter of conditional execution. Remember the trick of using the remainder (<Notebook code = {`%`}/>) operator for checking whether a number is divisible by another number (has a remainder of zero).

In the first version, there are three possible outcomes for every number, so you’ll have to create an <Notebook code = {`if`}/>/<Notebook code = {`else if`}/>/<Notebook code = {`else`}/> chain.

The second version of the program has a straightforward solution and a clever one. The simple solution is to add another conditional “branch” to precisely test the given condition. For the clever solution, build up a string containing the word or words to output and print either this word or the number if there is no word, potentially by making good use of the <Notebook code = {`||`}/> operator.

### **Chessboard**

Write a program that creates a string that represents an 8×8 grid, using newline characters to separate lines. At each position of the grid there is either a space or a "#" character. The characters should form a chessboard.

Passing this string to <Notebook code = {`console.log`}/> should show something like this:
...
When you have a program that generates this pattern, define a binding <Notebook code = {`size = 8`}/> and change the program so that it works for any <Notebook code = {`size`}/>, outputting a grid of the given width and height.

// Your code here.

You can build the string by starting with an empty one (<Notebook code = {`""`}/>) and repeatedly adding characters. A newline character is written <Notebook code = {`"\n"`}/>.
...
You’ll need two bindings to track your progress. To know whether to put a space or a hash sign at a given position, you could test whether the sum of the two counters is even (<Notebook code = {`% 2`}/>).
