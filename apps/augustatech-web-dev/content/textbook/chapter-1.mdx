...
<Sandbox code = {`let luigisDebt = 140;
...
<Sandbox code = {`prompt("Enter passcode");`}/>

![A prompt dialog that says 'enter passcode'](https://nbjrajrmujlgxmcvqsge.supabase.co/storage/v1/object/public/strapi/files/2-1-prompt.png-8ceb654c4df66a176f3138ca78a5ea19.png)
...
Though binding names cannot contain period characters, `console.lo`g does have one. This is because `console.log` isn’t a simple binding, but an expression that retrieves the `log` property from the value held by the `console` binding. We’ll find out exactly what this means in Chapter 4.
</section>
...
<Sandbox code = {`let theNumber = Number(prompt("Pick a number"));
...
![Diagram showing a straight arrow](https://nbjrajrmujlgxmcvqsge.supabase.co/storage/v1/object/public/strapi/files/2-2-controlflow-straight.svg-a3e3034abbe58c2c59ada45b67e9964e.svg)
</section>
...
![alt="Diagram of an arrow that splits in two, and then rejoins again"](https://nbjrajrmujlgxmcvqsge.supabase.co/storage/v1/object/public/strapi/files/2-3-controlflow-if.svg-ee294c8069b422f4f4f349d52cc2f89f.svg)
...
<Sandbox code = {`let num = Number(prompt("Pick a number"));
if (num < 10) {
 console.log("Small");
} else if (num < 100) {
 console.log("Medium");
...
![alt="Diagram showing arrow that splits in two, with on the branches splitting again, before all branches rejoin again"](https://nbjrajrmujlgxmcvqsge.supabase.co/storage/v1/object/public/strapi/files/2-4-controlflow-nested-if.svg-9a871b16fe2e8b548cadd147eb11bd2a.svg)
</section>
...
![Diagram showing an arrow to a point which has a cyclic arrow going back to itself and another arrow going further](https://nbjrajrmujlgxmcvqsge.supabase.co/storage/v1/object/public/strapi/files/2-5-controlflow-loop.svg-7d489d1b98224d28fec7d0cdad6d7997.svg)
...
<Sandbox code = {`for (let number = 0; number <= 12; number = number + 2) {
 console.log(number);
}`}/>
...
<Sandbox code = {`for (let current = 20; ; current = current + 1) {
 if (current % 7 == 0) {
   console.log(current);
   break;
 }
...
<Sandbox code = {`switch (prompt("What is the weather like?")) {
 case "rainy":
...
You can start with a program that prints out the numbers 1 to 7, which you can derive by making a few modifications to the [even number printing example](https://eloquentjavascript.net/02_program_structure.html#loops)given earlier in the chapter, where the `for` loop was introduced.
