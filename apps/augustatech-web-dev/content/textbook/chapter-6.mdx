...
![4-1.jpg](https://nbjrajrmujlgxmcvqsge.supabase.co/storage/v1/object/public/strapi/files/4-1.jpg-b2cf68cd377d2b5bcb12689f39c2c6ab.jpg)
...
![4-2.svg](https://nbjrajrmujlgxmcvqsge.supabase.co/storage/v1/object/public/strapi/files/4-2.svg-320938f78366a35dd31ad352542ee830.svg)
...
```
console.log("coconuts".slice(4, 7));// → nutconsole.log("coconut".indexOf("u"));// → 5
```
...
The `zeroPad` function from the [previous chapter](https://eloquentjavascript.net/03_functions.html) also exists as a method. It is called `padStart` and takes the desired length and padding character as arguments:
...
We have already seen the string type’s `length` property. Accessing the individual characters in a string looks like accessing array elements (with a complication that we’ll discuss in [Chapter 5](https://eloquentjavascript.net/05_higher_order.html#code_units)).
