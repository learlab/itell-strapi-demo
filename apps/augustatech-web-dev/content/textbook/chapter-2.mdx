...
<Sandbox pageSlug="test-page-no-chunks" code={`const halve = function(n) {
  return n / 2;
};
...
<Sandbox pageSlug="test-page-no-chunks" code={`console.log("The future says:", future());
...
<Sandbox pageSlug="test-page-no-chunks" code={`const square1 = (x) => { return x * x; };
const square2 = x => x * x;`}/>
...
<Sandbox pageSlug="test-page-no-chunks" code={`function minus(a, b) {
...
<Sandbox pageSlug="test-page-no-chunks" code={`function roundTo(n, step = 1) {
 let remainder = n % step;
 return n - remainder + (remainder < step / 2 ? 0 : step);
};
...
<Sandbox pageSlug="test-page-no-chunks" code={`function wrapValue(n) {
 let local = n;
 return () => local;
}
...
<Sandbox pageSlug="test-page-no-chunks" code={`function multiplier(factor) {
...
<Sandbox pageSlug="test-page-no-chunks" code={`function power(base, exponent) {
 if (exponent == 0) {
   return 1;
 } else {
...
<Sandbox pageSlug="test-page-no-chunks" code={`function findSolution(target) {
...
<Sandbox pageSlug="test-page-no-chunks" code={`function printFarmInventory(cows, chickens) {
...
<Sandbox pageSlug="test-page-no-chunks" code={`function printZeroPaddedWithLabel(number, label) {
...
<Sandbox pageSlug="test-page-no-chunks" code={`function zeroPad(number, width) {
