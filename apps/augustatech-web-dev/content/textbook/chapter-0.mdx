...
There are, of course, situations where you want a backslash in a string to be just a backslash, not a special code. If two backslashes follow each other, they will collapse together, and only one will be left in the resulting string value. This is how the string “_A newline character is written like_ `_"_`_\\n_`_"_`_._” can be expressed:
...
String values have a number of associated functions (_methods_) that can be used to perform other operations on them. I’ll say more about these in [Data Structures](https://itell-augustatech-web-dev.vercel.app/4-data-structures-objects-and-arrays).
...
<Info title="Try it yourself">
Below is a code sandbox for running JavaScript code directly in your browser. Compared to static code snippets, the sandbox is designed for interactive exploration of the code samples in this textbook. You can edit the sample code, click the run button, and view the output. You can also format and reset the editor to the initial state. As you are introduced more advanced topics, the sandbox will become more helpful and assist you in understanding how difference pieces in a program fit together. However, please note that the sandbox is only an approximation of the actual JavaScript runtime in the browser, and might not produce the exact same output in a few edge cases. For a fully featured runtime, consider using tools such as \[Chrome DevTools\](https://developer.chrome.com/docs/devtools/console) and \[Node.js\](https://nodejs.org/en).
</Info>

<Sandbox pageSlug="test-page-no-chunks" code={`console.log(typeof 4.5)
// → number
console.log(typeof "x")
// → string`}/>

We will use `console.log` in example code to indicate that we want to see the result of evaluating something. (More about that in the [next chapter](/https://itell-augustatech-web-dev.vercel.app/2-program-structure).)
...
<Sandbox pageSlug="test-page-no-chunks" code={`console.log(- (10 - 2))`}/>
</section>
...
In the [Introduction](https://eloquentjavascript.net/00_intro.html), I mentioned2 that JavaScript goes out of its way to accept almost any program you give it, even programs that do odd things. This is nicely demonstrated by the following expressions:

<Sandbox pageSlug="test-page-no-chunks" code={`console.log(8 * null)
// → 0
console.log("5" - 1)
// → 4
console.log("5" + 1)
