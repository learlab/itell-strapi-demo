...
```javascript caption="the minus operator can be used both as a binary operator and as a unary operator"
console.log(- (10 - 2))
```
...
<Sandbox pageSlug="test-page-no-chunks" code={`console.log(null || "user")
// → user
...
```javascript caption="?? only returns the right value if the left is exactly null or undefined"
console.log(0 || 100);
// → 100
console.log(0 ?? 100);
// → 0
