...
export function Blockquote({
  ...
  return (
    <figure
      className={cn("mx-auto max-w-screen-md text-center", className)}
      {...rest}
    >
      <svg
        aria-hidden="true"
      ...
      <figcaption className="mt-4 flex items-center justify-center space-x-3 text-lg">
        <div className="flex items-center divide-x-2">
          {author ? <cite className="pr-3 font-medium">{author}</cite> : null}
          {role ? (
            <cite className="pl-3 text-muted-foreground">{role}</cite>
          ) : null}
        </div>
      </figcaption>
    </figure>
