...
export function driver(options: Config = {}) {
	...
	function handleArrowLeft() {
		...
		const onPrevClick =
			activeStep.popover?.onPrevClick || getConfig("onPrevClick");
		if (onPrevClick) {
	...
	function handleArrowRight() {
		...
		const onNextClick =
			activeStep.popover?.onNextClick || getConfig("onNextClick");
		if (onNextClick) {
	...
	function drive(stepIndex = 0) {
		...
		const doneBtnText =
			currentStep.popover?.doneBtnText || getConfig("doneBtnText") || "Done";
		...
		const progressText =
			currentStep.popover?.progressText ||
		...
		const configuredButtons =
			currentStep.popover?.showButtons || getConfig("showButtons");
		const calculatedButtons: AllowedButtons[] = [
			...
			return (
				...
				configuredButtons.includes(b as AllowedButtons)
			);
		}) as AllowedButtons[];

		const onNextClick =
			currentStep.popover?.onNextClick || getConfig("onNextClick");
		const onPrevClick =
			currentStep.popover?.onPrevClick || getConfig("onPrevClick");
		const onCloseClick =
			currentStep.popover?.onCloseClick || getConfig("onCloseClick");
...
export { removeInert, setInertBackground };
