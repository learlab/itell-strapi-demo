...
import { easeInOutQuad, fixHighlight } from "./utils";
...
export function trackActiveElement(element: Element) {
	...
	let definition = element.getBoundingClientRect();
	if (definition.width === 0 || definition.height === 0) {
		const el = fixHighlight(element);
		definition = el.getBoundingClientRect();
	}
